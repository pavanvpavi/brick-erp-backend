...
public class PdfService {
        ...
        public byte[] generatePurchaseOrderPdf(PurchaseOrder po, String warehouseName) throws Exception {
                ByteArrayOutputStream out = new ByteArrayOutputStream();
                Document doc = new Document(PageSize.A4, 40, 40, 40, 40);
                PdfWriter.getInstance(doc, out);
                doc.open();
                ...
                leftCell.addElement(new Paragraph("Warehouse: " + warehouseName, normalFont()));
                infoTable.addCell(leftCell);
        ...
        public byte[] generateDeliveryChallanPdf(DeliveryOrder delivery) throws Exception {
                ...
                for (var item : delivery.getSalesOrder().getItems()) {
                        addTableRow(itemsTable, new String[] {
                                        ...
                                        item.getProduct().getName(),
                                        String.valueOf(item.getQuantity()),
                                        item.getProduct().getUnitOfMeasure() != null
                                                        ? item.getProduct().getUnitOfMeasure().getAbbreviation()
                                                        : "PCS"
                        }, sno % 2 == 0 ? LIGHT_GRAY : WHITE);
                }
                doc.add(itemsTable);
